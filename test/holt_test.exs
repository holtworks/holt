defmodule HoltTest do
  ...
  alias Holt.Runtime.{AgentEvents, Runs, Session, StateMachine}

  test "version is available" do
    assert Holt.version() == "0.1.2"
  end

  test "native command boundary reports unsupported commands" do
    output =
      capture_io(:stderr, fn ->
        assert Bridge.NativeCommand.run(%{"command" => "nope", "params" => %{}}) == 64
      end)

    assert output =~ "Unsupported Holt command."
  end
  ...
  test "onboard creates global config and workspace files" do
    ...
    assert output =~ "Holt is ready"
    assert output =~ "Provider check:"
    assert output =~ "Local service:"
  ...
  test "runtime chat mode completes a run without writing a planning artifact" do
    ...
    assert Enum.any?(progress_events, &(Map.get(&1, "type") == "progress.started"))
    assert Enum.any?(progress_events, &(Map.get(&1, "type") == "progress.reading"))
    assert Enum.any?(progress_events, &(Map.get(&1, "type") == "progress.thinking"))
  ...
  test "runtime chat mode summarizes the workspace for repo overview requests" do
    %{home: home, workspace: workspace} = tmp_env()
    Config.bootstrap(home: home)
    Workspace.init(workspace)
    ...
    File.write!(
      Path.join([workspace, "test", "app_test.exs"]),
      "defmodule Example.AppTest do\nend\n"
    )

    assert {:ok, %{run: run, artifact: artifact, output: output}} =
             Runtime.run("read this repo",
               home: home,
               workspace: workspace,
               approval: :always_approve,
               mode: "chat"
  ...
  test "runtime chat mode uses prior chat context for broad follow-up requests" do
    ...
    assert {:ok, %{artifact: artifact, output: output}} =
             Runtime.run("the entire project",
               home: home,
               workspace: workspace,
               approval: :always_approve,
               mode: "chat",
  ...
  test "runtime chat mode preserves model final output instead of forcing a plan heading" do
    %{home: home, workspace: workspace} = tmp_env()
    Config.bootstrap(home: home)
    Workspace.init(workspace)

    model_chat = fn _provider, messages, _opts ->
      send(self(), {:chat_mode_messages, messages})
    ...
    assert run["status"] == "completed"
    assert artifact == nil
    assert output == "Hello. What would you like to work on?"
    refute File.exists?(Path.join(workspace, "NEXT_STEPS.md"))
  ...
  test "native command logs render product tool history by default" do
    ...
    output =
      capture_io(fn ->
        assert Bridge.NativeCommand.run(%{
                 "command" => "logs",
                 "params" => %{"home" => home, "workspace" => workspace}
               }) == 0
      end)
  ...
  test "native command logs json emits machine-readable tool events" do
    ...
    output =
      capture_io(fn ->
        assert Bridge.NativeCommand.run(%{
                 "command" => "logs",
                 "params" => %{"json" => true, "home" => home, "workspace" => workspace}
               }) == 0
      end)
  ...
  test "search_web action records structured research claims" do
    ...
    web_search = fn %{"query" => "holtworks runtime"}, _opts ->
      {:ok,
       %{
         "answer" => "Holt is a local agent runtime.",
         "results" => [
           %{
             "title" => "Holt docs",
             "url" => "https://holtworks.ai/docs/runtime",
             "content" => "Runtime documentation for Holt."
           }
         ]
       }}
    end
    ...
    assert {:ok, execution} =
             Tasks.execute_action(
               ...
               %{
                 ...
                 "claim" => "Holt has local agent runtime documentation.",
    ...
    assert claim["claim"] == "Holt has local agent runtime documentation."
  ...
  test "task memory context budgets and continuation packets preserve exact evidence" do
    ...
    packet_path = Holt.Tasks.TaskMemory.context_packets_path(workspace)
  ...
  test "agent profiles cards and lifecycle feed task dispatch" do
    ...
    agent_names =
      Tasks.agents(workspace: workspace)
      |> Enum.map(& &1["display_name"])

    assert "Alpha Builder" in agent_names
  ...
  test "tool visibility metadata summarizes inputs without content leakage" do
    ...
    assert event["input_summary"]["path"] == "lib/app.ex"
  ...
  test "list files hides Holt working files from project summaries" do
    %{workspace: workspace} = tmp_env()
    Workspace.init(workspace)
  ...
  test "native command llm smoke test works with local provider" do
    %{home: home} = tmp_env()
  ...
  test "native command llm smoke test reports missing openrouter key clearly" do
    %{home: home} = tmp_env()
    ...
    output =
      capture_io(:stderr, fn ->
        assert Bridge.NativeCommand.run(%{
                 "command" => "llm_test",
                 "params" => %{"provider_id" => "openrouter", "home" => home}
               }) == 78
      end)
  ...
  test "openrouter adapter returns provider failure details" do
    ...
    post_json = fn _url, _headers, _body, _api_key ->
      {:ok,
       %{
         status: 400,
         body: %{"error" => %{"message" => "Invalid schema for function todos"}}
       }}
    end

    assert {:error,
            {:provider_request_failed, "openrouter", 400, "Invalid schema for function todos"}} =
             Models.chat(
               %{
                 "id" => "openrouter",
                 "type" => "openrouter",
  ...
  test "native command run completes with explicit approval flag" do
    %{home: home, workspace: workspace} = tmp_env()
  ...
  test "stdio executes task tools through the action layer" do
    ...
    assert %{"ok" => true, "result" => actions} =
             Bridge.Stdio.handle_request(%{"method" => "actions/list", "params" => %{}}, opts)

    assert Enum.any?(actions, &(&1["name"] == "add_comment"))
    ...
    assert Enum.any?(updated_task["comments"], &(&1["body"] == "via bridge action"))
  end
