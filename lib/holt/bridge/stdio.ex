defmodule Holt.Bridge.Stdio do
  @moduledoc """
  Minimal JSON-lines stdio bridge for local clients.
  """

  alias Holt.{Memory, Runtime, Tasks}
  alias Holt.Bridge.Stdio.{Agents, Params, Response, Sessions}
  alias Holt.Bridge.Stdio.Actions, as: ActionRequests
  alias Holt.Bridge.Stdio.RuntimeInfo
  alias Holt.Bridge.Stdio.Skills, as: SkillRequests
  ...
  def handle_request(%{"method" => "agent_sessions/start", "params" => params}, opts) do
    Sessions.start(params, opts)
  end

  def handle_request(%{"method" => "agent_sessions/status", "params" => params}, opts) do
    Sessions.status(params, opts)
  end

  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["agent_sessions/respond", "agent_sessions/resume"] do
    Sessions.respond(params, opts)
  end

  def handle_request(%{"method" => "agent_sessions/list", "params" => params}, opts) do
    Sessions.list(params, opts)
  end

  def handle_request(%{"method" => "agent_sessions/list"}, opts) do
    Sessions.list(opts)
  end

  def handle_request(%{"method" => "actions/list"}, opts) do
    ActionRequests.definitions(opts)
  end

  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["agent_events/list", "agent_events"] do
    Sessions.events(params, opts)
  end

  def handle_request(%{"method" => "agent_events/summary", "params" => params}, opts) do
    Sessions.summary(params, opts)
  end

  def handle_request(%{"method" => "agent_events/tree", "params" => params}, opts) do
    Sessions.tree(params, opts)
  end

  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["actions/list", "list_actions", "search_actions"] do
    ActionRequests.search(params, opts)
  end
  ...
  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["actions/catalog", "agent_actions/catalog", "action_catalog"] do
    ActionRequests.catalog(params, opts)
  end

  def handle_request(%{"method" => method}, opts)
      when method in ["actions/catalog", "agent_actions/catalog", "action_catalog"] do
    ActionRequests.catalog(%{}, opts)
  end

  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["agent_actions/openai", "actions/openai_action_definitions"] do
    ActionRequests.agent_definitions(params, opts)
  end

  def handle_request(%{"method" => method}, opts)
      when method in ["agent_actions/openai", "actions/openai_action_definitions"] do
    ActionRequests.agent_definitions(%{}, opts)
  end

  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["actions/providers", "agent_actions/providers"] do
    ActionRequests.provider_metadata(params, opts)
  end

  def handle_request(%{"method" => method}, opts)
      when method in ["actions/providers", "agent_actions/providers"] do
    ActionRequests.provider_metadata(%{}, opts)
  end

  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in [
             "actions/provider_prompt_sections",
             "agent_actions/provider_prompt_sections"
           ] do
    ActionRequests.provider_prompt_sections(params, opts)
  end

  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["agent_actions/dispatch", "dispatch_agent_action"] do
    ActionRequests.dispatch(params, opts)
  end

  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["actions/get", "get_action", "get_action_schema"] do
    ActionRequests.get(params, opts)
  end

  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["actions/execute", "execute_action"] do
    ActionRequests.execute(params, opts)
  end
  ...
  def handle_request(
    ...
    RuntimeInfo.meta_learning_snapshot(params)
  end

  def handle_request(%{"method" => "agent_runtime/meta_learning_snapshot"}, _opts) do
    RuntimeInfo.meta_learning_snapshot()
  end
  ...
  def handle_request(
    ...
    RuntimeInfo.format_local_model_result(params)
  end
  ...
  def handle_request(
    ...
    RuntimeInfo.agent_loop_contract(params)
  end
  ...
  def handle_request(
    ...
    RuntimeInfo.lifecycle_complete(params)
  end
  ...
  def handle_request(%{"method" => method, "params" => params}, opts)
    ...
    handle_action_result(Tasks.execute_action(repair_action_name(method), params, opts))
  end

  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["agents/list", "list_agents"] do
    Agents.list(params, opts)
  end
  ...
  def handle_request(%{"method" => "research_claims/list", "params" => params}, opts) do
    claim_opts =
      opts
      |> maybe_put_opt(:task_ref, research_claim_task_ref(params))
    ...
    with {:ok, node_ref} <- graph_node_ref_param(params),
         {:ok, graph} <-
           Tasks.complete_task_graph_node(
             graph_id,
             node_ref,
             Map.drop(params, ["graph_id", "node_ref"]),
             opts
           ) do
  ...
  def handle_request(
        %{"method" => "tasks/action_session", "params" => %{"ref" => ref} = params},
        opts
      ) do
    case Tasks.action_session(ref, Map.delete(params, "ref"), opts) do
  ...
  def handle_request(
        %{"method" => "tasks/route_action", "params" => %{"ref" => ref} = params},
        opts
      ) do
    case Tasks.route_action(ref, Map.delete(params, "ref"), opts) do
  ...
  def handle_request(%{"method" => method, "params" => %{"ref" => ref} = params}, opts)
      when method in ["tasks/execute_action", "execute_action"] do
    with {:ok, action_name} <- required_param(params, "action") do
      args = Map.get(params, "arguments", %{})

      handle_action_result(Tasks.execute_task_action(ref, action_name, args, opts))
    else
      {:error, reason} -> %{"ok" => false, "error" => inspect(reason)}
    end
  end
  ...
  def handle_request(%{"method" => method, "params" => %{"ref" => ref} = params}, opts)
      when method in ["tasks/multi_execute_action", "multi_execute_action"] do
    with {:ok, calls} <- required_param(params, "calls") do
  ...
  def handle_request(
    ...
    with {:ok, request_id} <- required_param(params, "approval_request_id") do
      case Tasks.resolve_action_approval_request(
             request_id,
             Map.delete(params, "approval_request_id"),
             opts
           ) do
        {:ok, request} -> %{"ok" => true, "result" => request}
        {:error, reason} -> %{"ok" => false, "error" => inspect(reason)}
  ...
  def handle_request(%{"method" => "tasks/capability_registry", "params" => params}, _opts) do
    with {:ok, action_name} <- required_param(params, "action"),
         {:ok, entry} <- Tasks.capability_registry(action_name, params) do
  ...
  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in [
             "tasks/record_action_event",
             "record_action_event",
             "tasks/record_action_event",
             "record_action_event"
           ] do
    handle_record_agent_run_event(params, opts, "action")
  end
  ...
  def handle_request(%{"method" => "add_comment", "params" => params}, opts) do
    with {:ok, ref} <- task_ref_param(params),
         {:ok, body} <- required_param(params, "body"),
         {:ok, task} <-
           Tasks.add_comment(ref, body, opts) do
      %{"ok" => true, "result" => task}
    else
      {:error, reason} -> %{"ok" => false, "error" => inspect(reason)}
    end
  ...
  def handle_request(%{"method" => "get_task_spec", "params" => params}, opts) do
    with {:ok, spec_id} <- required_param(params, "spec_id") do
      spec_opts =
        opts
        |> Keyword.put(:task_ref, task_spec_scope(params))
        |> Keyword.put(:content_limit, Map.get(params, "content_limit"))
  ...
  def handle_request(%{"method" => "start_agent_work", "params" => params}, opts) do
    with {:ok, ref} <- task_ref_param(params),
         {:ok, result} <- Tasks.start_agent_work(ref, drop_ref_params(params), opts) do
  ...
  def handle_request(%{"method" => "action_session", "params" => params}, opts) do
    with {:ok, ref} <- task_ref_param(params),
         {:ok, session} <- Tasks.action_session(ref, drop_ref_params(params), opts) do
  ...
  def handle_request(%{"method" => "get_task_action_session", "params" => params}, opts) do
    handle_request(%{"method" => "action_session", "params" => params}, opts)
  end

  def handle_request(%{"method" => "route_action", "params" => params}, opts) do
    with {:ok, ref} <- task_ref_param(params),
         {:ok, route} <- Tasks.route_action(ref, drop_ref_params(params), opts) do
  ...
  def handle_request(%{"method" => "action_route", "params" => params}, opts) do
    handle_request(%{"method" => "route_action", "params" => params}, opts)
  end

  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["tasks/execute_action", "execute_action"] do
    with {:ok, ref} <- task_ref_param(params),
         {:ok, action_name} <- required_param(params, "action") do
      args = Map.get(params, "arguments", %{})
  ...
  def handle_request(%{"method" => method, "params" => params}, opts)
      when method in ["tasks/multi_execute_action", "multi_execute_action"] do
    with {:ok, ref} <- task_ref_param(params),
         {:ok, calls} <- required_param(params, "calls") do
      handle_action_result(Tasks.execute_task_actions(ref, calls, opts))
    else
      {:error, reason} -> %{"ok" => false, "error" => inspect(reason)}
    end
  ...
  def handle_request(%{"method" => "resolve_action_approval_request", "params" => params}, opts) do
    with {:ok, request_id} <- required_param(params, "approval_request_id"),
         {:ok, request} <-
  ...
  def handle_request(%{"method" => "action_availability", "params" => params}, _opts) do
    RuntimeInfo.action_availability(params)
  end
  ...
  defp task_ref_param(params) do
    Params.task_ref(params)
  end

  defp target_ref_param(params) do
    Params.target_ref(params)
  end

  defp graph_node_ref_param(params) do
    Params.graph_node_ref(params)
  end

  defp required_param(params, key), do: Params.required(params, key)

  defp drop_ref_params(params) do
    Params.drop_ref(params)
  end
  ...
  defp handle_action_result(result), do: Response.action(result)
  ...
  defp handle_record_agent_run_event(params, opts, event_type) when is_map(params) do
    with {:ok, run_id} <- agent_run_id_param(params) do
      params = Map.delete(params, "agent_run_id")
  ...
  defp record_agent_run_event("action", run_id, params, opts),
    do: Tasks.record_agent_run_action_event(run_id, params, opts)
  ...
  defp record_agent_run_event("plan_contract", run_id, params, opts),
    do:
      record_payload(
        params,
        "plan_contract",
        &Tasks.record_agent_run_plan_contract(run_id, &1, opts)
      )
  ...
  defp record_agent_run_event("child_contract", run_id, params, opts),
    do:
      record_payload(
        ...
        &Tasks.record_agent_run_child_contract(run_id, &1, opts)
      )
  ...
  defp record_agent_run_event("continuation_packet", run_id, params, opts),
    do:
      record_payload(
        params,
        "packet",
        &Tasks.record_agent_run_continuation_packet(run_id, &1, opts)
      )

  defp record_payload(params, key, fun) do
    with {:ok, payload} <- Params.required_map(params, key) do
      fun.(payload)
    end
  end
  ...
  defp agent_run_id_param(params) do
    required_param(params, "agent_run_id")
  end

  defp handle_process_started(params, opts) when is_map(params) do
    with {:ok, process} <- Params.required_map(params, "process"),
         {:ok, context} <- Params.required_map(params, "context"),
         {:ok, result} <-
           Tasks.record_process_started(
             process_payload(process, "running"),
             context,
             opts
           ) do
      %{"ok" => true, "result" => result}
    ...
    with {:ok, process} <- Params.required_map(params, "process"),
         ...
         {:ok, result} <-
           Tasks.notify_process_terminal(
             process_payload(process, "exited"),
             context,
             opts
           ) do
      %{"ok" => true, "result" => result}
  ...
  defp process_payload(process, default_status) do
    Map.put_new(process, "status", process_status(process, default_status))
  end

  defp research_claim_task_ref(params), do: Params.value(params, "task_ref")

  defp task_spec_scope(params), do: Params.value(params, "ref")

  defp process_status(params, default_status) do
    case Params.value(params, "status") do
      value when value in [nil, ""] -> default_status
      status -> status
    end
  end

  defp repair_action_name("repair_runs/start"), do: "start_repair_run"
  defp repair_action_name("repair_runs/get"), do: "get_repair_run"
  defp repair_action_name("repair_runs/record_artifact"), do: "record_repair_run_artifact"
  defp repair_action_name("repair_runs/reconcile_prediction"), do: "reconcile_repair_prediction"
  defp repair_action_name("repair_runs/score_predictions"), do: "score_repair_predictions"
  defp repair_action_name("repair_runs/choose_strategy"), do: "choose_repair_strategy"

  defp repair_action_name("repair_runs/draft_architecture_plan"),
    do: "draft_repair_architecture_plan"

  defp repair_action_name("repair_runs/draft_blast_radius"), do: "draft_repair_blast_radius"

  defp repair_action_name("repair_runs/draft_original_issue_check"),
    do: "draft_repair_original_issue_check"

  defp repair_action_name("repair_runs/execute_original_issue_check"),
    do: "execute_repair_original_issue_check"

  defp repair_action_name("repair_runs/execute_impact_check"), do: "execute_repair_impact_check"

  defp repair_action_name("repair_runs/draft_related_issue_sweep"),
    do: "draft_repair_related_issue_sweep"

  defp repair_action_name("repair_runs/begin_implementation"), do: "begin_repair_implementation"
  defp repair_action_name("repair_runs/approve_gate"), do: "approve_repair_gate"
  defp repair_action_name("repair_runs/complete"), do: "complete_repair_run"
  defp repair_action_name(method), do: method
