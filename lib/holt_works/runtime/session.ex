defmodule HoltWorks.Runtime.Session do
  @moduledoc """
  Supervised live session wrapper around `HoltWorks.Runtime.run/2`.

  Sessions provide the Inktrail-style runtime surface HoltWorks needs without
  ...
  alias HoltWorks.{Clock, Paths, Runtime, Workspace}
  alias HoltWorks.Runtime.SessionStore
  ...
  def start(objective, opts) when is_binary(objective) do
    ...
    case DynamicSupervisor.start_child(HoltWorks.Runtime.SessionSupervisor, child) do
      {:ok, _pid} ->
  ...
  def via(session_id), do: {:via, Registry, {HoltWorks.Runtime.SessionRegistry, session_id}}
  ...
  def handle_call({:respond, answer}, _from, state) do
    ...
    send(awaiting.waiter, {:holtworks_user_response, awaiting.ref, to_string(answer || "")})
  ...
  def handle_info(:run_session, state) do
    ...
    task =
      Task.Supervisor.async_nolink(HoltWorks.Runtime.SessionTaskSupervisor, fn ->
  ...
  defp lookup(session_id) do
    case Registry.lookup(HoltWorks.Runtime.SessionRegistry, session_id) do
      [{pid, _meta}] -> {:ok, pid}
  ...
  defp await_user(owner, question, metadata) do
    ...
    receive do
      {:holtworks_user_response, ^ref, answer} -> {:ok, answer}
    after
  ...
  defp broadcast(state, event) do
    Enum.each(Map.keys(state.subscribers), fn subscriber ->
      ...
      send(subscriber, {:holtworks_session_event, state.session_id, event})
    end)
