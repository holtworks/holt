defmodule HoltWorks.Tasks do
  @moduledoc """
  Workspace-local task flow with durable artifacts and run linkage.
  """

  alias HoltWorks.{Actions, Agents, Clock, JSON, Paths, ResearchClaims, Runtime}
  ...
  defp process_wake_message(task, packet) do
    ...
    """
    HoltWorks observed a previously awaited process terminal event. Continue the same task from this structured process wake packet.
  ...
  defp watchdog_recovery_message(task, packet) do
    ...
    """
    HoltWorks detected that the previous agent run is no longer making effective progress. Continue the same task from this structured watchdog recovery packet.
