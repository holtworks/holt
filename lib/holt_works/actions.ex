defmodule HoltWorks.Actions do
  @moduledoc """
  Executable action registry for local HoltWorks providers.
  ...
  dispatch, and workspace actions keep using the existing `HoltWorks.Tools`
  approval policy.
  """

  alias HoltWorks.{Clock, Pages, Tools}
  alias HoltWorks.Actions.{ProviderRegistry, ToolCatalog}
  ...
  defp route_action(ref_or_id, tool_name, args, opts) do
    ...
    if ref_or_id in [nil, ""] do
      ...
      HoltWorks.Tasks.route_task_tool(ref_or_id, attrs, opts)
    end
  end
  ...
  defp dispatch(_ref, "list_tasks", args, opts) do
    {:ok, HoltWorks.Tasks.list(task_list_opts(args, opts))}
  end

  defp dispatch(_ref, "create_task", args, opts),
    do: HoltWorks.Tasks.create(action_arguments(args), opts)

  defp dispatch(ref, "get_task", _args, opts), do: HoltWorks.Tasks.get(ref, opts)

  defp dispatch(ref, "update_task", args, opts),
    do: HoltWorks.Tasks.update(ref, action_arguments(args), opts)

  defp dispatch(ref, "set_priority", args, opts) do
    with {:ok, priority} <- required_any(args, ["priority"]) do
      HoltWorks.Tasks.set_priority(ref, priority, opts)
    end
  end

  defp dispatch(ref, "set_estimate", args, opts) do
    with {:ok, estimate} <- required_any(args, ["estimate"]) do
      HoltWorks.Tasks.set_estimate(ref, estimate, opts)
    end
  end
  ...
  defp dispatch(ref, "add_label", args, opts) do
    ...
    HoltWorks.Tasks.add_label(ref, label_args, opts)
  end

  defp dispatch(ref, "remove_label", args, opts) do
    with {:ok, name} <- required_any(args, ["name", "label"]) do
      HoltWorks.Tasks.remove_label(ref, name, opts)
    end
  end
  ...
  defp dispatch(_ref, "read_task_memory_artifact", args, opts) do
    with {:ok, artifact_ref} <- required_any(args, ["artifact_ref", "spec_id", "id"]) do
      HoltWorks.Tasks.read_memory_artifact(artifact_ref, opts)
    end
  end

  defp dispatch(ref, "save_teammate_memory", args, opts),
    do: HoltWorks.Tasks.save_teammate_memory(ref, action_arguments(args), opts)

  defp dispatch(ref, "load_teammate_runtime", args, opts),
    do: HoltWorks.Tasks.load_teammate_runtime(ref, teammate_runtime_opts(args, opts))

  defp dispatch(ref, "record_task_memory_artifact", args, opts),
    do: HoltWorks.Tasks.record_task_memory_artifact(ref, action_arguments(args), opts)

  defp dispatch(ref, "task_memory_context", args, opts),
    do: HoltWorks.Tasks.task_memory_context(ref, action_arguments(args), opts)

  defp dispatch(ref, "context_budget", args, opts),
    do: HoltWorks.Tasks.context_budget(ref, action_arguments(args), opts)

  defp dispatch(ref, "continuation_packet", args, opts),
    do: HoltWorks.Tasks.continuation_packet(ref, action_arguments(args), opts)

  defp dispatch(ref, "verifier_calibration", args, opts),
    do: HoltWorks.Tasks.verifier_calibration(ref, action_arguments(args), opts)

  defp dispatch(ref, "start_agent_work", args, opts),
    do: HoltWorks.Tasks.start_agent_work(ref, action_arguments(args), opts)

  defp dispatch(ref, "continue_agent_work", args, opts),
    do: HoltWorks.Tasks.continue_agent_work(ref, action_arguments(args), opts)

  defp dispatch(_ref, "watchdog_agent_runs", args, opts) do
    {:ok, HoltWorks.Tasks.watchdog_scan(watchdog_opts(args, opts))}
  end

  defp dispatch(ref, "create_task_graph", args, opts),
    do: HoltWorks.Tasks.create_task_graph(ref, action_arguments(args), opts)

  defp dispatch(ref, "list_task_graphs", _args, opts), do: HoltWorks.Tasks.task_graphs(ref, opts)

  defp dispatch(_ref, "get_task_graph", args, opts) do
    with {:ok, graph_id} <- required_any(args, ["graph_id", "task_graph_id", "id"]) do
      HoltWorks.Tasks.get_task_graph(graph_id, opts)
    end
  end

  defp dispatch(_ref, "advance_task_graph", args, opts) do
    with {:ok, graph_id} <- required_any(args, ["graph_id", "task_graph_id", "id"]) do
      HoltWorks.Tasks.advance_task_graph(
        graph_id,
  ...
  defp dispatch(_ref, "complete_task_graph_node", args, opts) do
    with {:ok, graph_id} <- required_any(args, ["graph_id", "task_graph_id", "id"]),
      ...
      HoltWorks.Tasks.complete_task_graph_node(
        graph_id,
        node_ref,
  ...
  defp dispatch(ref, "work_graph_budget", args, opts),
    do: HoltWorks.Tasks.work_graph_budget(ref, action_arguments(args), opts)

  defp dispatch(ref, "agent_dispatch_plan", args, opts),
    do: HoltWorks.Tasks.agent_dispatch_plan(ref, action_arguments(args), opts)

  defp dispatch(ref, "team_orchestration", args, opts),
    do: HoltWorks.Tasks.team_orchestration(ref, action_arguments(args), opts)

  defp dispatch(ref, "child_agent_contract", args, opts),
    do: HoltWorks.Tasks.child_agent_contract(ref, action_arguments(args), opts)

  defp dispatch(ref, "verifier_dispatch", args, opts),
    do: HoltWorks.Tasks.verifier_dispatch(ref, action_arguments(args), opts)

  defp dispatch(ref, "route_verification_review", args, opts),
    do: HoltWorks.Tasks.route_verification(ref, verification_args(args), opts)

  defp dispatch(ref, "get_evidence_contract", args, opts),
    do: HoltWorks.Tasks.evidence_contract(ref, action_arguments(args), opts)

  defp dispatch(ref, "plan_verifier_route", args, opts),
    do: HoltWorks.Tasks.plan_verifier_route(ref, action_arguments(args), opts)

  defp dispatch(ref, "work_graph", args, opts),
    do: HoltWorks.Tasks.work_graph(ref, action_arguments(args), opts)

  defp dispatch(ref, "work_graph_gate", args, opts),
    do: HoltWorks.Tasks.work_graph_gate(ref, action_arguments(args), opts)

  defp dispatch(ref, "work_graph_schedule", args, opts),
    do: HoltWorks.Tasks.work_graph_schedule(ref, action_arguments(args), opts)

  defp dispatch(ref, "verification_contract", args, opts),
    do: HoltWorks.Tasks.verification_contract(ref, action_arguments(args), opts)

  defp dispatch(ref, "verifier_assignment", args, opts),
    do: HoltWorks.Tasks.verifier_assignment(ref, action_arguments(args), opts)

  defp dispatch(ref, "action_contract", args, opts),
    do: HoltWorks.Tasks.action_contract(ref, action_arguments(args), opts)

  defp dispatch(ref, "plan_contract", args, opts),
    do: HoltWorks.Tasks.plan_contract(ref, action_arguments(args), opts)

  defp dispatch(ref, "plan_gate", args, opts),
    do: HoltWorks.Tasks.plan_gate(ref, action_arguments(args), opts)

  defp dispatch(ref, "action_preflight", args, opts),
    do: HoltWorks.Tasks.action_preflight(ref, action_arguments(args), opts)

  defp dispatch(ref, "consequence_gate", args, opts),
    do: HoltWorks.Tasks.consequence_gate(ref, action_arguments(args), opts)

  defp dispatch(ref, "action_runtime_envelope", args, opts),
    do: HoltWorks.Tasks.action_runtime_envelope(ref, action_arguments(args), opts)

  defp dispatch(_ref, "complete_action_runtime_envelope", args, _opts) do
    with {:ok, envelope} <- required_map(args, "envelope") do
      HoltWorks.Tasks.complete_action_runtime_envelope(
        envelope,
  ...
  defp dispatch(_ref, "capability_registry", args, _opts) do
    with {:ok, tool_name} <- required_any(args, ["tool_name", "name", "tool"]) do
      HoltWorks.Tasks.capability_registry(tool_name, action_arguments(args))
    end
  end

  defp dispatch(ref, "capability_contract", args, opts),
    do: HoltWorks.Tasks.capability_contract(ref, action_arguments(args), opts)

  defp dispatch(ref, "capability_route", args, opts),
    do: HoltWorks.Tasks.capability_route(ref, action_arguments(args), opts)

  defp dispatch(ref, "generic_plan", args, opts),
    do: HoltWorks.Tasks.generic_plan(ref, action_arguments(args), opts)

  defp dispatch(ref, "action_approval_request", args, opts),
    do: HoltWorks.Tasks.action_approval_request(ref, action_arguments(args), opts)
  ...
  defp dispatch(_ref, "resolve_action_approval_request", args, opts) do
    with {:ok, request_id} <- required_any(args, ["approval_request_id", "request_id", "id"]) do
      HoltWorks.Tasks.resolve_action_approval_request(
        request_id,
  ...
  defp dispatch(ref, "action_evidence_ledger", args, opts),
    do: HoltWorks.Tasks.action_evidence_ledger(ref, action_arguments(args), opts)
  ...
  defp description(tool_name, _tool),
    do: "Execute #{tool_name} through the local HoltWorks provider."
  ...
  defp argument_properties("start_repair_run") do
    Map.merge(base_argument_properties(), %{
      ...
      "risk_level" => %{"type" => "string", "enum" => HoltWorks.RepairRuns.risk_levels()},
  ...
  defp argument_properties("record_repair_run_artifact") do
    ...
    |> Map.merge(%{
      "artifact_type" => %{"type" => "string", "enum" => HoltWorks.RepairRuns.artifact_types()},
      "payload" => %{"type" => "object"}
    })
  end
  ...
  defp argument_properties("reconcile_repair_prediction") do
    ...
    |> Map.merge(%{
      ...
      "next_decision" => %{"type" => "string", "enum" => HoltWorks.RepairRuns.decisions()}
    })
  end
  ...
  defp argument_properties("choose_repair_strategy") do
    ...
    |> Map.merge(repair_run_identifier_properties())
    |> Map.merge(%{
      "strategy" => %{"type" => "string", "enum" => HoltWorks.RepairRuns.strategies()},
      "risk_level" => %{"type" => "string", "enum" => HoltWorks.RepairRuns.risk_levels()},
  ...
  defp argument_properties("list_agents") do
    Map.merge(base_argument_properties(), %{
      "status" => %{"type" => "string", "enum" => ["all"] ++ HoltWorks.Agents.statuses()},
      "status_filter" => %{"type" => "string", "enum" => ["all"] ++ HoltWorks.Agents.statuses()}
    })
  end
  ...
  defp argument_properties(tool_name)
    ...
    Map.merge(base_argument_properties(), %{
      "summary" => %{"type" => "string"},
      "category" => %{"type" => "string", "enum" => HoltWorks.Memory.user_categories()},
      "user_id" => %{"type" => "string"}
    })
  end
  ...
  defp argument_properties("remember_for_project") do
    Map.merge(base_argument_properties(), %{
      "summary" => %{"type" => "string"},
      "category" => %{"type" => "string", "enum" => HoltWorks.Memory.project_categories()},
      "project_id" => %{"type" => "string"}
    })
  end
  ...
  defp argument_properties("recall_project_memory") do
    Map.merge(base_argument_properties(), %{
      "query" => %{"type" => "string"},
      "kind" => %{"type" => "string", "enum" => HoltWorks.Memory.project_kinds()},
  ...
  defp argument_properties("search_web") do
    Map.merge(base_argument_properties(), %{
      ...
      "claim" => %{"type" => "string"},
      "source_type" => %{"type" => "string", "enum" => HoltWorks.ResearchClaims.source_types()},
  ...
  defp agent_profile_properties do
    %{
      ...
      "handle" => %{"type" => "string"},
      "agent_ref" => %{"type" => "string"},
      "status" => %{"type" => "string", "enum" => HoltWorks.Agents.statuses()},
      "work_roles" => %{
        "type" => "array",
